...
def _https_score(url: str) -> int:
    return 0 if url.lower().startswith("https://") else 35
...
def _collect_phrases(text: str, patterns: List, category: str) -> List[Dict[str, str]]:
    ...
    for pattern in patterns:
        ...
        for match in matches:
            ...
            raw_severity = meta.get("severity", 0.5)
            try:
...
class RiskEngine:
    ...
    def _heuristic_intent(self, text: str) -> Tuple[int, List[Dict[str, str]], str]:
        ...
        lowered = text.lower()
        intent_score = 0
        intent_score += 22 * len(manipulation_hits)
        intent_score += 16 * len(spoof_hits)
        intent_score += 14 * len(urgency_hits)
        # mild bump for long messages pushing actions
        if len(text) > 500:
            intent_score += 10
        if manipulation_hits and urgency_hits:
            intent_score += 12
        if "http" in lowered or "www." in lowered:
            intent_score += 8
        if "locked" in lowered and "verify" in lowered:
            intent_score += 10
    ...
    def _analyze_domain(self, url: str, allow_network: bool = False) -> DomainFinding:
        ...
        if domain.lower() not in KNOWN_BRANDS and typo_ratio >= 0.82:
        ...
        if scheme != "https":
            issues.append("Insecure scheme (HTTPS missing) — do not enter credentials")
        score += _https_score(url)

        sensitive_path_tokens = ("reset", "verify", "update", "login", "signin", "unlock", "secure", "mfa", "2fa")
        if any(token in path for token in sensitive_path_tokens):
            issues.append("Sensitive action embedded in URL path")
            score += 12
    ...
    def _content_risk(self, text: str) -> Tuple[int, List[Dict[str, str]]]:
        ...
        for hit in highlights:
            try:
                sev = float(hit.get("severity", 0.5) or 0.5)
            except (TypeError, ValueError):
                sev = 0.5
            score += max(14, int(28 * sev))
        if "gift card" in lowered or "crypto" in lowered:
            score += 20
        if "password" in lowered:
            score += 12
        if "locked" in lowered and "verify" in lowered:
            score += 15
        if "reset" in lowered and ("password" in lowered or "account" in lowered):
            score += 12
    ...
    def analyze(
        ...
        base_overall = (
            ...
            + WEIGHTS["intent"] * breakdown.intent_risk
        )

        combination_boost = 0
        if content_highlights and urgency_highlights:
            combination_boost += 12
        if detected_urls and (content_highlights or urgency_highlights):
            combination_boost += 8
        if spoof_highlights:
            combination_boost += 5
        if any(f.score >= 60 for f in domain_findings):
            combination_boost += 6

        overall = normalize_score(base_overall + combination_boost)

        lowered_text = text.lower()
        http_link_present = any(str(f.url).lower().startswith("http://") for f in domain_findings)
        risky_action_present = any(token in lowered_text for token in ("verify", "reset", "update", "login", "sign in", "unlock"))
        high_risk_combo = bool(detected_urls and risky_action_present and (content_highlights or urgency_highlights))
        if high_risk_combo and http_link_present:
            overall = max(overall, 90)
        elif high_risk_combo:
            overall = max(overall, 80)
        elif http_link_present and (content_highlights or urgency_highlights):
            overall = max(overall, 80)
